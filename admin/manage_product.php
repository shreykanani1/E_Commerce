...
if (isset($_FILES['uploadimg'])) {
	$uploadimg = uploadImage($_FILES['uploadimg'], './productimgs/');
}
...
if(isset($_POST['submit'])){
	...
	if(isset($_GET['id']) && $_GET['id']!=''){
		if($_FILES['uploadimg']['name']!=''){
			...
			$update_sql="update product set categories_id='$categories_id',name='$name',mrp='$mrp',price='$price',qty='$qty',short_desc='$short_desc',description='$description',meta_title='$meta_title',meta_desc='$meta_desc',meta_keyword='$meta_keyword',image='$uploadimg' where id='$id'";
		}else{
		...
		// $image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
		// move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
		mysqli_query($con,"insert into product(categories_id,name,mrp,price,qty,short_desc,description,meta_title,meta_desc,meta_keyword,status,image) values('$categories_id','$name','$mrp','$price','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$uploadimg')");
	}
	header('location:product.php');
	die();
							...
							<div class="card-body card-block">
							   <div class="form-group">
								...
								<div class="form-group">
									...
									<input type="file" name="uploadimg" class="form-control" <?php echo  $image_required?>>
								</div>
